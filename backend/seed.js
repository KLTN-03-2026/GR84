...
function generateUsers(count = 50) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    users.push({
      ...
      status: 'active',
      // Trải ngẫu nhiên trong 30 ngày qua để biểu đồ tăng trưởng có dữ liệu nhiều ngày
      createdAt: new Date(Date.now() - randomBetween(0, 30) * 86400000 - randomBetween(0, 86400000))
    });
  }
...
function generateTags() {
  ...
  const tagsOccupations = occupations.map(name => ({
    name,
    category: 'occupation',
    status: 'active',
  ...
  const tagsLocations = locations.map(name => ({
    name,
    category: 'location',
    status: 'active',
...
async function seedDatabase() {
  try {
    ...
    await Promise.all([
      ...
      AdminLog.collection.deleteMany({}),
      Role.deleteMany({}),
      UserRole.deleteMany({}),
      UserSession.deleteMany({})  // Xóa session cũ để tránh đếm nhầm
    ]);
    ...
    // timestamps: false để giữ nguyên createdAt tùy chỉnh (không bị Mongoose ghi đè)
    const users = await User.insertMany(usersWithHashedPassword, { timestamps: false });
    ...
    for (const tag of tags) {
      ...
      if (tag.category === 'interest') {
        ...
        count = users.filter(u => u.location === tag.name).length;
      }
    ...
    for (let i = 0; i < 15; i++) {
      ...
      adminLogs.push({
        ...
        adminName: randomAdmin.fullName || randomAdmin.username || 'Admin',
        adminRole: randomAdmin.role || 'admin',
        action: getRandomItem(actions),
        status: Math.random() > 0.1 ? 'Thành công' : 'Thất bại',
        targetId: target._id,
